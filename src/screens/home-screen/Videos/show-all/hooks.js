...
import { getListVideos, likeItem } from '../../actions';
...
const useDesignersFacade = (t) => {
  ...
  const { videos, loading, nextPageVideos, currentPageVideo } = useSelector(state => state.home)
  ...
  const _onLoadMore = () => {
    if (nextPageVideos && nextPageVideos > currentPageVideo) {
      if (location) {
        const data = {
          page: nextPageVideos,
          limit: 15,
