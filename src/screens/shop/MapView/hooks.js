...
const useMapViewFacade = (editableLocation, t) => {
  ...
  useEffect(() => {
    if (editableLocation) {
      const temp = editableLocation.split(',');
      _onInitMapPreviewUrl(temp)
      _onGetAdress(temp)
      setLocation(temp)
      const marker = {
        latitude: temp[0] ? temp[0] : 51.507351,
        longitude: temp[1] ? temp[1] : -0.127758
      }
  ...
  useEffect(() => {
    if (location && location.length > 0 && !editableLocation) {
